...
# initialize
x = np.array([1, 2, 3, 4, 5, 6])  # feature
y = np.array([0]) # label
...
for data in data_set:
    ...
    for sceneInfo in data[3:-1]: #get the feature you need
    ...
    x = np.vstack((x, np.hstack((Ball_x, Ball_y, direction, Blocker_x, Ball_speed_x, Ball_speed_y))))
...
if not os.path.isdir(path):
    os.mkdir(path)
with open(os.path.join(path,"save","model.pickle"), 'wb') as f:
    pickle.dump(model, f)